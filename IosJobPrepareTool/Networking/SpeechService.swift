...
/// this does the networking ,fetch voice data from google tts api.
/// i copied from internet. should refactor a little.
...
let APIKey = "AIzaSyDWVJB_rqrit_09DclkYvFX97qu7EdRntc"
...
class SpeechService: NSObject, AVAudioPlayerDelegate {
    ...
    private var loopPlay = UserDefaults.standard.bool(forKey: "loopPlay")
    private var voice = UserDefaults.standard.string(forKey: "voice")
    
    func update() {
        self.voice = UserDefaults.standard.string(forKey: "voice")
    }
    ...
    func speak(text: String, voiceType: VoiceType = .waveNetFemale, completion: @escaping () -> Void) {
        ...
        DispatchQueue.global(qos: .background).async {
            ...
            DispatchQueue.main.async {
                ...
                self.loopPlay = UserDefaults.standard.bool(forKey: "loopPlay")
                if self.loopPlay {
